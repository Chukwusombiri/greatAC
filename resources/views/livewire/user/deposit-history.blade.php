    ...
    <div class="w-full px-3">
        <div class="bg-gradient-to-br from-gray-950 via-gray-900 to-gray-950 border border-gray-600 shadow-2xl rounded-2xl overflow-hidden">
            ...
            <div class="flex flex-wrap items-center justify-between p-6 border-b border-gray-700">
                ...
                <x-link-two href="{{ route('user.deposit.pricingTable') }}" class="rounded-xl text-sm md:text-base">
                    + New Deposit
                </x-link-two>
            </div>
            ...
            <div class="w-full px-6 py-8">
                <div class="relative w-full flex items-center bg-gray-900 border border-primary/20 rounded-full shadow-md focus-within:border-primary transition">
                    <input 
                        type="text" 
                        wire:model="search"
                        placeholder="Search by plan, wallet, or amount..."
                        class="w-full bg-transparent text-primary-light px-5 py-3 rounded-full focus:outline-none placeholder:text-primary-muted"
                    >
                    <button wire:click="clear" type="button" class="absolute right-3 text-primary-muted hover:text-primary-light transition">
                    ...
                    <tbody>
                        @forelse ($deposits as $deposit)
                            <tr class="border-b border-gray-800 hover:bg-gray-900/50 transition">
                                <td class="px-6 py-4 font-semibold text-primary-lighter">${{ number_format($deposit->amount) }}</td>
                                <td class="px-6 py-4 text-center text-primary">{{ $deposit->plan }}</td>
                                <td class="px-6 py-4 text-primary-light">{{ $deposit->wallet }}</td>
                                ...
                                <td class="px-6 py-4">
                                    @if ($deposit->receipt)
                                        <a href="{{ asset('storage/' . $deposit->receipt) }}" target="_blank"
                                           class="text-sm text-blue-600 hover:text-blue-500 transition underline-offset-2 hover:underline">
                                           View Receipt
                                        </a>
                                    @else
